...
maxlen=120
def test_imp(instances, cnn_imp_dict, cnn_imp_model):
    nb_filter, filter_length1, filter_length2, filter_length3 = 1024,3,3,3
    lr = 0.001
    activation = 'tanh'
    batch_size = 128
    rf = open(cnn_imp_dict,'r')
    vocab = json.load(rf)
    w2i_dic = vocab[0]
    p2i_dic = vocab[1]
    ...
    model.load_weights(cnn_imp_model)
...
def test_nop(instances, cnn_noexp_dict, cnn_noexp_model):
    nb_filter, filter_length1, filter_length2, filter_length3 = 512 ,4,8,12
    lr = 0.001
    activation = 'tanh'
    batch_size = 64
    rf = open(cnn_noexp_dict,'r')
    vocab = json.load(rf)
    w2i_dic = vocab[0]
    p2i_dic = vocab[1]
    Arg1_word,Arg2_word,Arg1_pos,Arg2_pos = embedding_process(instances, w2i_dic, p2i_dic)
    ...
    model.load_weights(cnn_noexp_model)
    dev_output = model.predict({'arg1': Arg1_word, 'arg2': Arg2_word, 'pos1': Arg1_pos, 'pos2': Arg2_pos},
                                       batch_size=batch_size)['output']
    # print(dev_output)
    return dev_output
