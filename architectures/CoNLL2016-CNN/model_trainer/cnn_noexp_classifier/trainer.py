...
from model_trainer.cnn_noexp_classifier import evaluation
#import evaluation
import sys
import math
...
def pretainning():
    ...
    def data_process(relations_file,parses_file):
        ...
        # c2={}
        # for r in relation:
        #     if len(r[0]) not in c1.keys():
        #         c1[len(r[0])]=1
        #     else:
        #         c1[len(r[0])]+=1
        #     if len(r[1]) not in c2.keys():
        #         c2[len(r[1])]=1
        #     else:
        #         c2[len(r[1])]+=1
        # print(c1)
...
if __name__ == '__main__':
    ...
    activation = "tanh"
    batch_size = 64
    lr ,loss_weight= 0.001, 0
    # nb_filter, filter_length1, filter_length2, filter_length3 = 512,2,4,8
    nb_filter = 1500
    filter_list = [(3,3,3),(2,4,5),(2,3,3),(2,5,10)]
    for filter_length1, filter_length2, filter_length3 in filter_list:
        ...
        best_epoch, best_acc_test, dev_epoch_th, dev_best_acc = \
                fit_model(
                  ...
                  WE, pos_WE, word_dim, pos_dim,
                  X_train_1, X_train_2, X_train_pos_1, X_train_pos_2, y_train, y_train_type,
                  X_dev_1, X_dev_2, X_dev_pos_1, X_dev_pos_2, y_dev, y_dev_type,loss_weight,train=True)
        print(best_epoch, best_acc_test, dev_epoch_th, dev_best_acc)
        print(nb_filter,filter_length1, filter_length2, filter_length3,dev_best_acc)
