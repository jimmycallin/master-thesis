...
def pretainning(data_base_path, word_embedding_path, test_type="dev"):
    threshold_vocab = 0

    pos_ndims = 100
    maxlen = 120
    trian_file_path = data_base_path + "-train/"
    dev_file_path = data_base_path + "-" + test_type + "/"
    ...
    w2i_dic = {}
    p2i_dic = {}
    w2v_file = word_embedding_path
    wv = Word2Vec.load_word2vec_format(w2v_file, binary=False)
    ndims = wv.syn0.shape[1]
...
def pretesting(input_path, word_embedding_path, train_file_path):
    threshold_vocab = 0

    pos_ndims = 100
    maxlen = 120

    dev_file_path = input_path + "/"
    parses_file_name = "parses.json"
    relations_file_name = "relations.json"
    dev_parses = dev_file_path + parses_file_name
    dev_relations = dev_file_path + relations_file_name
    train_relations = train_file_path + relations_file_name
    train_parses = train_file_path + parses_file_name
    vocab = set()
    pos_vocab = set()
    w2i_dic = {}
    p2i_dic = {}
    ...
    def embedding_process(relations,parses):
        data = data_process(relations,parses)
        tmp = []
        for x in data:
            arg1 = []
            arg2 = []
        ...
        X_pos_1 = sequence.pad_sequences(X_pos_1, maxlen=maxlen, padding='pre', truncating='pre')
        X_pos_2 = sequence.pad_sequences(X_pos_2, maxlen=maxlen, padding='post', truncating='post')
        print(relations,parses, (X_1.shape, X_pos_1.shape, y_s.shape, y_t.shape))
        return (X_1, X_2, X_pos_1, X_pos_2, y_s, y_t)

    WE, pos_WE = WE_process()
    X_dev_1, X_dev_2, X_dev_pos_1, X_dev_pos_2, y_dev, y_dev_type = embedding_process(dev_relations,dev_parses)
    print("the lens of vocab and pos_vocab: ",len(vocab),len(pos_vocab))
    return (WE, pos_WE, len(vocab), len(pos_vocab), X_dev_1, X_dev_2, X_dev_pos_1, X_dev_pos_2, y_dev, y_dev_type)


def fit_model(lr, activation, nb_filter, filter_length1, filter_length2, filter_length3,
              WE, pos_WE, word_dim, pos_dim,
              X_train_1, X_train_2, X_train_pos_1, X_train_pos_2, y_train, y_train_type,
              X_dev_1, X_dev_2, X_dev_pos_1, X_dev_pos_2, y_dev, y_dev_type,train=True, store_path="./model.h5"):
    batch_size = 128
    nb_epoch = 80
    # callback each epoch:
    class scorer(Callback):
        ...
        def on_epoch_end(self, epoch, logs={}):
            ...
            if acc_dev > scorer.dev_best_acc:
                ...
                model.save_weights(store_path, overwrite=True)
            if epoch % 3 == 0:
                print('the best dev is {}:'.format(store_path.split('/')[-1]), nb_filter, filter_length1, filter_length2, filter_length3, 'epoch', scorer.dev_epoch_th, 'dev_best_acc:', scorer.dev_best_acc)
            print()
            sys.stdout.flush()
...
def test(lr, activation, nb_filter, filter_length1, filter_length2, filter_length3,
          WE, pos_WE, word_dim, pos_dim,batch_size,model_path, train):
    model = build_model(lr, activation, nb_filter, filter_length1, filter_length2, filter_length3,
...
def data_process_special(input_path):
    rf = open(input_path + "/relations.json")
    relations = [json.loads(x) for x in rf]
    parse_dict = json.load(codecs.open(input_path + "/parses.json", encoding='utf8'))
    relation = []
    flag= 0
    for r in relations:
        docid = r['DocID']
...
def test_and_store(lr, activation, nb_filter, filter_length1, filter_length2, filter_length3,
          WE, pos_WE, word_dim, pos_dim,batch_size,model_path,input_path,train_file_path,output_dir,train):
    model = build_model(lr, activation, nb_filter, filter_length1, filter_length2, filter_length3,
          WE, pos_WE, word_dim, pos_dim,train)
    model.load_weights(model_path)
    dev_output = model.predict({'arg1': X_dev_1, 'arg2': X_dev_2, 'pos1': X_dev_pos_1, 'pos2': X_dev_pos_2},
                                       batch_size=batch_size)['output']
    instances = data_process_special(input_path)
    relations = [i[5] for i in instances]
    for i,r in enumerate(relations):
        r['Arg1']['TokenList'] = [t[2] for t in r['Arg1']['TokenList']]
        r['Arg2']['TokenList'] = [t[2] for t in r['Arg2']['TokenList']]
        r["Sense"] = [cnn_config.Label_To_Sense[np.argmax(dev_output[i])]]

    with open(output_dir + '/output.json', 'w') as output:
        for relation in relations:
            output.write(json.dumps(relation) + '\n')
...
if __name__ == '__main__':
    if sys.argv[5] == 'train':
        ...
        WE, pos_WE, word_dim, pos_dim, \
        X_train_1, X_train_2, X_train_pos_1, X_train_pos_2, y_train, y_train_type, \
        X_dev_1, X_dev_2, X_dev_pos_1, X_dev_pos_2, y_dev, y_dev_type = pretainning(data_base_path, word_embedding_path)
        activation = "tanh"
        batch_size = 128
        lr = 0.001
        final = []
        ...
        filter_length1, filter_length2, filter_length3 = (3, 3, 3)
        '''  train '''
        ...
        best_epoch, best_acc_test, dev_epoch_th, dev_best_acc = \
            fit_model(
              ...
              WE, pos_WE, word_dim, pos_dim,
              X_train_1, X_train_2, X_train_pos_1, X_train_pos_2, y_train, y_train_type,
              X_dev_1, X_dev_2, X_dev_pos_1, X_dev_pos_2, y_dev, y_dev_type,train=True, store_path=model_store_path + "/" + name + ".h5")
        ...
        print((filter_length1, filter_length2, filter_length3,dev_best_acc))
        ...
        train_file_path = sys.argv[7]
        WE, pos_WE, word_dim, pos_dim, \
        X_dev_1, X_dev_2, X_dev_pos_1, X_dev_pos_2, y_dev, y_dev_type = pretesting(input_path, word_embedding_path, train_file_path)
        activation = "tanh"
        batch_size = 128
        lr = 0.001
        final = []
        ...
        test_and_store(lr, activation, nb_filter, filter_length1, filter_length2, filter_length3,
             WE, pos_WE, word_dim, pos_dim,batch_size, model_path=model_store_path + "/" + name + ".h5", input_path=input_path, train_file_path=train_file_path, output_dir=output_dir, train=False)
    else:
        raise ValueError("fifth argument has to be 'train' or 'test'")
